...
//    }
    ...
    suspend fun saveInvitation(inv: InvitationFlmEntity): Result<String> {
        return try {
            ...
            ref.set(inv.toFirebaseMap(ref.id)).await()
            Result.success(ref.id)
        } catch (e: Exception) {
            Result.failure(e)
        }
    ...
    suspend fun fetchFormations(): List<FormationEntity> {
        return try {
            ...
            for (doc in querySnap.documents) {
                ...
                val formateur  = doc.getString("formateur")        ?: ""
                val dateAppreciation = doc.getString("dateAppreciation") ?: ""

                // New fields
                val entite      = doc.getString("entite")      ?: ""
                val categorie   = doc.getString("categorie")   ?: ""
                val division    = doc.getString("division")    ?: ""
                val convocation = doc.getString("convocation") ?: ""
                val presence    = doc.getString("presence")    ?: ""
                val session     = doc.getString("session")     ?: ""
                val jsp         = doc.getString("jsp")         ?: ""
                val type        = doc.getString("type")        ?: ""
                val domaine     = doc.getString("domaine")     ?: ""
                ...
                val collab  = (collabRaw as? String)?.takeIf { it.isNotBlank() }
    ...
    suspend fun UploadForm(roomId:Long,form: Forms){
        try {
            firestore.collection("forms")
                ...
                .set(form.copy(id = roomId).toMap())
                .await()
        } catch (e: Exception) {
            // Firestore upload failed → silently ignored
        }
    }
...
fun FormationEntity.toMap() = mapOf(
    ...
    "debut"                  to debut,
    "fin"                    to fin,
    "formateur"              to Formateur,
    "dateAppreciation"       to dateAppreciation,
    "entite"                 to entite,
    "categorie"              to categorie,
    "division"               to division,
    "convocation"            to convocation,
...
fun InvitationFlmEntity.toFirebaseMap(firebaseId: String) = mapOf(
    "firebaseId"             to firebaseId,
    "formationId"            to formationId,
    "datesFormation"         to datesFormation,
    "formateur"              to formateur,
    "matriculeCollaborateur" to matriculeCollaborateur,
    "nomCompletCollaborateur" to nomCompletCollaborateur,
    "service"                to service,
    "themeNom"               to themeNom,
    "themeObjectives"        to themeObjectives,
    "emailFlm"               to emailFlm,
    "nomFlm"                 to nomFlm,
    "statut"                 to statut.name,
    "dateEnvoi"              to dateEnvoi
)
